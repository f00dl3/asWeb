...
function processMosData(last, heightsIn, hours, runs, jsonModelData) {
    ...
    var lastStringSanitized = runString.replace("_"," ").replace("Z","");
    ...
    var runDateTime = dojo.date.add(djLastString, "hour", 0);
    var urlPrefix = formatDate(runDateTime, "yyyy-MM-dd HH");
    urlPrefix = urlPrefix.replace(" ", "_"); // format to Ymd_H for image files!
    ...
    gfsFh.forEach(function (tfhA) {
        var tfh = tfhA.FHour;
        if(isSet(tfh)) {
            var thisDjLastString = dojo.date.add(djLastString, "hour", tfh);
            var tValidTime = formatDate(thisDjLastString, "yyyy-MM-dd HH");
            tAutoCounter = 0;
            var tAuto0TF = 0;
            ...
            if(isSet(tAuto0TF)) {
                mosTable += "<div class='tr'>" +
                        "<span class='td'>" + tValidTime + "z</span>" +
