...
function common_isSet(what) {
	console.log("\nDBG --> function common_isSet(" + what + ")");
	if(typeof what !== 'undefined' && what) {
		return true;
	} else {
		return false;
...
function common_parseDate(oH) {
	
	console.log("\nDBG --> function common_parseDate(" + oH + ")");
	
	var di = new Date();
	if(common_isSet(oH)) { di = new Date(di.setHours(di.getHours()- oH)); }
	var dY = di.getFullYear();
	var dM = ("0" + (di.getMonth()+1)).slice(-2);
	var dD = ("0" + (di.getDate())).slice(-2);
	var dh = di.getHours();
