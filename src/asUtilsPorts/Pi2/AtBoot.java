...
public class AtBoot {
    ...
    private static void startVideo() {
        ...
        final String basicCamCap = "ffmpeg -i /dev/video0 -update 1 /var/www/http/stream.jpg > /dev/null 2>&1";
        //final String start_v4l2ctl = "v4l2-ctl --set-ctrl=exposure_time_absolute=220";
        //final String start_h264server = "v4l2rtspserver -W 954 -H 540 -F 15 -P 8554 /dev/video0 &";
        //try { sj.runProcess(start_v4l2ctl); } catch (Exception e) { e.printStackTrace(); }
        try { wc.runProcess(basicCamCap); } catch (Exception e) { e.printStackTrace(); }
        //try { sj.runProcess(start_h264server); } catch (Exception e) { e.printStackTrace(); }
