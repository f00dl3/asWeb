...
public class ModelWorkerCMC_19 {
	...
	public static void main(String[] args) {
		...
		try ( ResultSet resultSetCFH = mdb.q2rs1c(dbc, cFHq, null); ) {
            while (resultSetCFH.next()) { cFHList.add(resultSetCFH.getString("FHour")); } 
        } catch (Exception e) { e.printStackTrace(); }

		try ( ResultSet resultSetVcmc = mdb.q2rs1c(dbc, vCMCq, null); ) {
            while (resultSetVcmc.next()) { vCMC.add(resultSetVcmc.getString("VarName")); } 
        } catch (Exception e) { e.printStackTrace(); }

		try ( ResultSet resultSetVLcmc = mdb.q2rs1c(dbc, vlCMCq, null); ) { 
		...
		for (String tFHourStr : cFHList) {
			...
			for(String tVar : vlCMC) {
				for(String tHeight : heightList) {
					...
					StumpJunk.jsoupOutBinary(thisFHData, thisFHFile, wmb.getDownloadTimeout());
				}
			}
